...
async function getStargazerTimesForPage(
	...
	const dates: Date[] = data.map(
		(stargazer: any) => new Date(stargazer.starred_at),
	);
...
async function getTimes(
	...
	for (let page = 1; page <= totalPages; page++) {
		stargazerPromises.push(getStargazerTimesForPage(repo, page, headers));
	}

	const stargazerTimes = await Promise.all(stargazerPromises);

	// In case of failure, return the first error
	const objectsThatFailed = stargazerTimes.filter(
	...
	const stargazerMs = stargazerTimes
		.map(({ stargazerTimes }) => stargazerTimes)
		.flat();
