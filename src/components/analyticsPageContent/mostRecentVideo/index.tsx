...
type PaginationButtonProps =
  | ({ disabled: true } & ComponentPropsWithoutRef<"button">)
  | ({ disabled?: false } & ComponentProps<typeof Link>);
...
export function MostRecentVideo({ id, thumbnail, title, url, views }: Props) {
  ...
  return (
    <div className="w-full rounded-sm border border-border p-3 text-sm">
      ...
      <div className="mb-6 flex flex-col items-center space-y-1 ">
        <a className="w-full" href={url} rel="noreferrer" target="_blank">
          <img
            alt="thumbnail"
